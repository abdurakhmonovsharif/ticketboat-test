...
from app.model.persona_account.persona_account_service_interface import PersonaAccountService, PersonaSyncResult
...
class TicketSuitePersonaAccountService(PersonaAccountService):
    """
    ...
    """
    def __init__(self, persona_account_client: Type[PersonaAccountClient]):
        super().__init__(persona_account_client)
        self._client_class = persona_account_client  # Api client class, instantiated with api_key when needed
    ...
    async def sync_account_to_automator(
        ...
        """
        Sync account's primaries to a single TicketSuite automator.
        
        This is the main entry point called by PersonaOrchestrator.
        ...
        try:
            # Get account data with primaries
            account_primaries = await get_accounts_with_primaries([account_id])
            
            if not account_primaries:
            ...
            account = account_primaries[0]
            
            # Validate account has email_id
            if not account.email_id:
                error_msg = f"Account {account_id} has no email_id"
            ...
            # Build persona data list from account's primaries
            primary_ids_set = set(primary_ids)
            persona_data_list = []
            skipped_primaries = []  # Track skipped primaries for response
            account_dict = self._build_account_dict(account)
            
            for primary in account.primaries:
                # Only include primaries that are in the requested list
                if primary.id not in primary_ids_set:
                    continue
                
                # Skip primaries that don't pass validation (already added to TS, missing fields)
                if self._should_skip_primary(primary):
                    # Build skip reason
                    skip_reasons = []
                    if primary.added_to_ts:
                        skip_reasons.append("already added to TicketSuite")
                    if primary.missing_fields:
                        skip_reasons.append(f"missing fields: {primary.missing_fields}")
                    
                    skip_reason = "; ".join(skip_reasons) if skip_reasons else "validation failed"
                    
                    logger.info(
                        f"Skipping primary {primary.primary_name} for account {account_id}: "
                        f"added_to_ts={primary.added_to_ts}, missing_fields={primary.missing_fields}"
                    )
                    
                    # Add to skipped list for response
                    skipped_primaries.append({
                        "account_id": account_id,
            ...
            # If no valid primaries, return with skipped info
            if not persona_data_list:
                logger.info(f"No valid primaries to sync for account {account_id}")
                return PersonaSyncResult(
                    automator_id=automator_id,
                    automator_name=automator_name,
                    account_id=account_id,
                    successful=[],
                    failed=skipped_primaries  # Include skipped primaries in response
                )
            
            # Create personas in TicketSuite
            result = await self._create_personas_in_ticketsuite(
                automator_id=automator_id,
                automator_name=automator_name,
                api_key=api_key,
            ...
            # Combine API failures with skipped primaries
            all_failed = result.failed + skipped_primaries
            
            # Convert PersonaCreationResult to PersonaSyncResult
            return PersonaSyncResult(
            ...
            error_msg = f"Error syncing account {account_id} to TicketSuite: {str(e)}"
            logger.error(error_msg)
            traceback.print_exc()
            return PersonaSyncResult(
                automator_id=automator_id,
    ...
    async def _create_personas_in_ticketsuite(
        ...
        """
        try:
            # Instantiate the client class with the api_key
            client_api = self._client_class(api_key=api_key)
            ...
            async with client_api:
                batch_results = await client_api.create_in_batch(personas_to_create)
...
def get_persona_creation_service() -> TicketSuitePersonaAccountService:
    """
    Get the global TicketSuitePersonaAccountService instance.
    
    Returns a fresh instance per call to avoid shared mutable state
    between concurrent operations.
    
    Uses the factory to get the properly configured service.
    """
    from app.model.persona_account.persona_account_factory import PersonaAccountFactory
    return PersonaAccountFactory.get_service('ticketsuite')
